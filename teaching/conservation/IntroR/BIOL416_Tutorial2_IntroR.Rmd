...
# By giving a new column name after a $, R creates a
#    column called "new.column" to the iris data.frame
iris.2$new.column <- 0
head(iris.2)
...
*Ask class to come up with equation to calculate mean - should take 10 seconds or so, but will get them thinking about math*. 
...
*Demonstrate looking at the help file and searching for help with `?`*
...
In R, we can use different functions to load up our own personal data-sets - but we'll get into those later.
For the moment, let's return to the iris data set.
...
```{r}
##### Instead of reading in the data here, instead let's load the iris file from a .csv
##### Brief demo with getwd()
head(v)
head(iris)
# View(iris)
summary(iris)
...
*Quickly demonstrate how to save plots using the GUI with the Plots tab*.
...
*Get the students to make a totally new type of plot - `boxplot()` - and save it to their computer. Provide only a sample plot and the name of the `boxplot()` function*. 
...
*Ask class if anyone knows what statistical test could be used here*. 
...
*Brief conversation about whether the regression actually means anything given the species level differences*. 

##### END HERE

Finally, we probably also want to know if the relationship between these two variables is statistically significant. 
...
Finally, just like with other objects, we can pull out individual datum from this object. 

For example, to save the p-value of the Sepal.Length coefficient returned by the model fitting:

```{r}
lm.anova$`Pr(>F)`
# Note that the NA is because there is no P value for the Residuals in the model - residuals measure the extra noise that is not captured by the linear model.
```

Don't forget to save your work, and see you next week for more about R and how to use it to manipulate and work with phylogenies.

## Summary

- objects - 0d, 1d, 2d
- indexing objects:
   - with `$` if the components of the object are named or in **lists**
   - with `[]` in all cases; if 2d, use multiple indexes with `[,]`
- operations - math, comparisons
- functions - more complex operations that are often pre-compiled
- plotting - many different types, generally you pass on graphical parameters within the `plot()` call itself or, in some cases, after `plot()` has been called
- statistics - R can perform many, many statistical analyses - many simple statistical tools are available right away in R, and many others are available through libraries (which we'll discuss next week)
